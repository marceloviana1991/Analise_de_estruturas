def diag_cort(restricoes_de_apoio, rotula, carregamento, caracteristicas, comprimento, angulo,
              deslocabilidades, i, j):
    import math
    ...
    for a in range(len(comprimento)):
        if not(rotula[a][0]) and not(rotula[a][1]):
            caso_0.append((-carregamento_local[a][0] * comprimento[a] / 2) * math.sin(angulo[a]) +
