def diag_mom(carregamento, comprimento, angulo, rotula, caracteristicas, restricoes_de_apoio, deslocabilidades, i, j):
    import math
    ...
    for a in range(len(carregamento)):
        carregamento_local.append(0)
    # Conversão dos carregamentos que atuam nas barras para o sistema de coordenadas locais
    for a in range(len(carregamento)):
        carregamento_local[a] = -carregamento[a][0] * math.sin(angulo[a]) + carregamento[a][1] * math.cos(angulo[a])
    # Caso 0
    for a in range(len(carregamento)):
        if not(rotula[a][0]) and not(rotula[a][1]):
            caso_0.append(carregamento_local[a] * comprimento[a] ** 2 / 12)
            caso_0.append(-carregamento_local[a] * comprimento[a] ** 2 / 12)
        elif rotula[a][0] and not(rotula[a][1]):
            caso_0.append(0)
            caso_0.append(-carregamento_local[a] * comprimento[a] ** 2 / 8)
