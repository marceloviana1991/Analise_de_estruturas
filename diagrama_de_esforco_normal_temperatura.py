def diag_nor(restricoes_de_apoio, rotula, caracteristicas, comprimento, angulo, deslocabilidades, alfa,
             temperatura, cg, h, i, j):
    import math
    ...
    for a in range(len(comprimento)):
        ...
        if not (rotula[a][0]) and not (rotula[a][1]):
            caso_0.append(caracteristicas[a][2] * caracteristicas[a][0] * alfa[a] * tg * math.cos(angulo[a]))
            caso_0.append(- caracteristicas[a][2] * caracteristicas[a][0] * alfa[a] * tg * math.cos(angulo[a]))
